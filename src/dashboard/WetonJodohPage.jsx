  ...
  return (
    <section className="mx-auto max-w-[1180px] px-5 py-7 sm:px-7 lg:py-9">
      <header className="mb-7 text-center">
        ...
        <p className="mt-2 text-sm text-[#8FA7B8]">Hitung Kecocokan Jodoh berdasarkan weton (hari dan pasaran).</p>
      </header>
...
function PersonSummary({ title, person, accent = 'text-emerald-300' }) {
  return (
    <Card title={title}>
      <div className="text-base font-bold text-white">{person?.weton || '—'} <span className={`ml-2 rounded border border-current/30 px-2 py-1 text-[10px] ${accent}`}>Neptu {person?.neptu ?? '—'}</span></div>
...
function SymbolCard({ title, person, feminine = false }) {
  return (
    <Card title={title}>
      ...
      <div className="mt-1 text-center text-xs text-[#8298A8]">Lintang / Lambang Weton</div>
      <div className="mt-5 grid grid-cols-2 gap-2">
        <div className="rounded-lg bg-[#07111C] p-4 text-center"><div className="text-xl">◎</div><div className="mt-2 text-xs font-semibold text-white">Wuku</div><div className="mt-1 text-[10px] text-[#71899A]">{person?.wuku || '—'}</div></div>
        <div className="rounded-lg bg-[#07111C] p-4 text-center"><div className="text-xl">♡</div><div className="mt-2 text-xs font-semibold text-white">Pasaran</div><div className="mt-1 text-[10px] text-[#71899A]">{person?.pasaran || '—'}</div></div>
      </div>
      <div className="mt-5"><div className="text-xs font-bold text-white">Watak</div><p className="mt-2 text-xs leading-5 text-[#91A7B7]">Data watak belum ditambahkan ke engine Jodoh; bagian ini mengikuti struktur referensi dan tidak mengarang interpretasi baru.</p></div>
    </Card>
  )
}

function AnalysisCard({ title, person }) {
  return (
    <Card title={title}>
      <div className="rounded-lg border-t-2 border-amber-300 bg-[#07111C] p-4">
        <div className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#6F8798]">Petungan Wolu (Mod 8)</div>
        <div className="mt-2 text-sm font-bold text-amber-300">{person?.jodohName || '—'}</div>
        <p className="mt-2 text-xs leading-5 text-[#A6B8C5]">{person?.jodohMeaning || 'Hasil petungan Wolu berdasarkan Neptu Weton.'}</p>
      </div>
      <div className="mt-2 grid gap-2 sm:grid-cols-2">
        <div className="rounded-lg bg-[#07111C] p-4"><div className="text-[9px] font-bold uppercase tracking-[0.1em] text-[#6F8798]">Rakam</div><div className="mt-1 text-sm font-semibold text-white">{person?.rakam || '—'}</div></div>
        <div className="rounded-lg bg-[#07111C] p-4"><div className="text-[9px] font-bold uppercase tracking-[0.1em] text-[#6F8798]">Pancasudha</div><div className="mt-1 text-sm font-semibold text-white">{person?.pancaSudha || '—'}</div></div>
      </div>
    </Card>
  )
}
...
function JodohResultPage() {
  ...
  useEffect(() => {
    ...
    getProfile().catch(() => null).then(async (profile) => {
      ...
      setPeople({ one: { ...one, jodohName: jodoh.name, jodohMeaning: jodoh.meaning }, two: { ...two, jodohName: jodoh.name, jodohMeaning: jodoh.meaning } })
      setResult(jodoh)
  ...
  const score = result?.name === 'Pesthi' ? 90 : result?.name === 'Jodoh' ? 90 : result?.name === 'Ratu' ? 85 : result?.name === 'Tinari' ? 85 : 70
  ...
  return (
    <section className="mx-auto max-w-[1180px] px-5 py-7 sm:px-7 lg:py-9">
      <header className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-[#8FA7B8]">Pembacaan kecocokan berdasarkan pertemuan Neptu Weton kedua pihak.</p>
      </header>
      ...
      <div className="rounded-xl border border-white/[0.08] bg-[#0A1723] p-5 text-center shadow-[0_12px_40px_rgba(0,0,0,0.14)] sm:p-8">
        ...
        <div className="mt-5 text-[10px] font-bold uppercase tracking-[0.18em] text-[#72899A]">Hasil Utama · Petungan Wolu (Modulo 8)</div>
        <div className={`mx-auto mt-4 inline-flex rounded-lg border px-5 py-2 text-3xl font-bold ${tone}`}>{result?.name}</div>
        <p className="mx-auto mt-3 max-w-2xl text-sm text-[#A7BAC7]">{result?.meaning}</p>
        <div className="mx-auto mt-6 max-w-[760px] overflow-hidden rounded-full bg-[#162633]"><div className="h-3 rounded-full bg-amber-300" style={{ width: `${score}%` }} /></div>
        <div className="mt-3 text-3xl font-bold text-amber-200">{score}%</div>
        <div className="text-xs text-[#748A9A]">Skor ringkas kecocokan menurut pembacaan petungan</div>
        <div className="mt-5 border-t border-white/[0.08] pt-5 text-left text-xs text-[#8FA7B8]"><span className="font-mono text-cyan-200">{people.one.neptu} + {people.two.neptu} = {total}</span><span className="float-right uppercase tracking-[0.12em] text-[9px]">Total Neptu Pasangan · Modulo 8</span></div>
      </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <AnalysisCard title="Analisis Petungan — Pihak Pertama" person={people.one} />
        <AnalysisCard title="Analisis Petungan — Pihak Kedua" person={people.two} />
      </div>
      ...
      <Card title="☀ Analisis Petungan Pasangan" className="mt-4">
        ...
        <p className="mt-2 text-xs text-[#8298A8]">ⓘ Petungan di bawah melengkapi hasil utama; untuk pernikahan, perhatikan khususnya Wolu.</p>
        <div className={`mt-4 rounded-lg border p-4 ${tone}`}><div className="text-[9px] font-bold uppercase tracking-[0.1em]">Petungan Wolu (Mod 8) — Pernikahan</div><div className="mt-2 text-base font-bold">{result?.name}</div><div className="mt-1 text-xs">{result?.meaning}</div></div>
      </Card>

      <Card title="☀ Tradisi Wariga Bali" className="mt-4">
        <div className="grid gap-5 md:grid-cols-[0.8fr_1.2fr] md:items-center">
          <div className="text-center"><div className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#6F8798]">Patemon / Panca Sudha</div><div className="mt-3 text-xl font-bold text-amber-300">{people.one.bali?.detail?.pancaSudha || people.one.pancaSudha || '—'}</div><div className="mt-2 text-xs text-[#91A7B7]">Referensi Bali dari kalender existing.</div></div>
          <div className="space-y-2 text-xs leading-5 text-[#A6B8C5]"><div><strong className="text-white">Total Neptu pasangan:</strong> {total}</div><div><strong className="text-white">Panca Sudha pihak pertama:</strong> {people.one.pancaSudha}</div><div><strong className="text-white">Panca Sudha pihak kedua:</strong> {people.two.pancaSudha}</div><div><strong className="text-white">Rakam pihak pertama:</strong> {people.one.rakam}</div><div><strong className="text-white">Rakam pihak kedua:</strong> {people.two.rakam}</div></div>
        </div>
      </Card>

      <Card title="● Nasihat Primbon" className="mt-4">
        <p className="text-xs leading-6 text-[#A6B8C5]"><strong className="text-white">Petungan Wolu (pernikahan):</strong> {result?.meaning}</p>
        <p className="mt-2 text-xs italic leading-6 text-[#8FA7B8]">Hasil ini merupakan referensi petungan tradisional, bukan penentu mutlak hubungan. Variasi metode dan sumber primbon dapat menghasilkan pembacaan yang berbeda.</p>
      </Card>
      ...
      <details className="mt-4 rounded-xl border border-white/[0.08] bg-[#0A1723]">
        ...
        <div className="border-t border-white/[0.08] px-5 py-4 text-xs leading-6 text-[#91A7B7]"><div>Neptu pihak pertama: <strong className="text-white">{people.one.neptu}</strong></div><div>Neptu pihak kedua: <strong className="text-white">{people.two.neptu}</strong></div><div>Total: <strong className="text-white">{total}</strong></div><div>Modulo 8: <strong className="text-white">{result?.remainder}</strong></div><div className="mt-2">{result?.method}</div></div>
      </details>
