  ...
  return (
    <section className="mx-auto max-w-[1180px] px-5 py-7 sm:px-7">
      <div className="mb-7">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Perhitungan Palelintangan berdasarkan siklus Pawukon dan 35 Lintang.</p>
      </div>
      <div className="grid gap-5 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="rounded-2xl border border-white/[0.07] bg-[#0A1723] p-5">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            <label className="block"><span className="mb-2 block text-xs text-slate-400">Tanggal</span><input type="date" className="w-full rounded-xl border border-white/[0.08] bg-[#07111C] px-4 py-3 text-sm text-white outline-none" /></label>
            <label className="block"><span className="mb-2 block text-xs text-slate-400">Waktu</span><input type="time" className="w-full rounded-xl border border-white/[0.08] bg-[#07111C] px-4 py-3 text-sm text-white outline-none" /></label>
          </div>
          <button type="button" disabled className="mt-5 w-full rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-sm font-semibold text-slate-500">Hitung Palelintangan</button>
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-[#0A1723] p-5">
          ...
          <div className="mt-5 rounded-xl border border-cyan-400/20 bg-cyan-400/[0.04] p-5"><div className="text-xs uppercase tracking-[0.14em] text-cyan-400">Lintang</div><div className="mt-2 text-2xl font-semibold text-white">—</div><div className="mt-2 text-sm text-slate-400">Masukkan tanggal kelahiran untuk melihat hasil.</div></div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2">{SECTIONS.map(([title, description]) => (<div key={title} className="rounded-xl border border-white/[0.07] bg-[#07111C] p-4"><div className="text-sm font-semibold text-white">{title}</div><div className="mt-1 text-xs leading-5 text-slate-500">{description}</div><div className="mt-3 text-sm text-slate-400">—</div></div>))}</div>
        </div>
      </div>
