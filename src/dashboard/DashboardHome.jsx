...
export default function DashboardHome({ showFooter = true }) {
  ...
  const data = {
    ...
    natural: { ...(apiData?.natural || context.natural || {}), sun, moon: naturalResults.find((r) => r.id === "moon")?.data || naturalResults.find((r) => r.id === "moon") || null, eclipse: naturalResults.find((r) => r.id === "eclipse")?.data || naturalResults.find((r) => r.id === "eclipse") || null, sky: naturalResults.find((r) => r.id === "sky")?.data || naturalResults.find((r) => r.id === "sky") || null, earth: naturalResults.find((r) => r.id === "earth-space")?.data || naturalResults.find((r) => r.id === "earth-space") || null, tide: naturalResults.find((r) => r.id === "tide")?.data || naturalResults.find((r) => r.id === "tide") || null },
    schedule: apiData?.schedule || schedule,
    quick_jumps: apiData?.quick_jumps || { today: context.mode === "live" && context.selectedDate ? iso(context.selectedDate) : null, next_full_moon: naturalResults.find((result) => result.id === "moon")?.nextFullMoon ?? null, next_eclipse: eclipseData.next ?? null },
  ...
  return (
    <div className={`min-h-screen font-sans antialiased transition-colors duration-700 ${isNight ? "bg-[#0F172A] text-white" : "bg-[#F8FAFC] text-[#0F172A]"}`} data-cakra-mode={isNight ? "night" : "day"}>
      ...
      <main className="min-w-0 flex-1">
        <AppContent data={data} dateISO={iso(context.selectedDate)} onSelectDate={(value) => context.setSelectedDate(new Date(`${value}T12:00:00`))} time={context.selectedTime.slice(0, 5)} onTimeChange={handleTimeChange} onJumpToday={context.goLive} quickJumps={data.quick_jumps} onOpenWeton={() => setWetonOpen(true)} onOpenEclipse={() => setEclipseOpen(true)} />
      </main>
