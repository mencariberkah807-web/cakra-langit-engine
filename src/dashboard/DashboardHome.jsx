...
export default function DashboardHome({ showFooter = true, user }) {
  ...
  const data = {
    ...
    natural: { ...(apiData?.natural || context.natural || {}), sun, moon: moonResult?.data || moonResult || null, eclipse: naturalResults.find((r) => r.id === "eclipse")?.data || naturalResults.find((r) => r.id === "eclipse") || null, sky: naturalResults.find((r) => r.id === "sky")?.data || naturalResults.find((r) => r.id === "sky") || null, earth: naturalResults.find((r) => r.id === "earth-space")?.data || naturalResults.find((r) => r.id === "earth-space") || null, tide: apiData?.natural?.tide || naturalResults.find((r) => r.id === "tide")?.data || naturalResults.find((r) => r.id === "tide") || null },
