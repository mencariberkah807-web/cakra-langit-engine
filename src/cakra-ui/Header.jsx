  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/[0.07] bg-[#07111C]/90 backdrop-blur-xl">
      <div className="mx-auto grid max-w-[1440px] gap-4 px-4 py-4 sm:px-6 lg:px-8 xl:flex xl:items-center">
        ...
        <div className="grid w-full min-w-0 grid-cols-2 items-center gap-4 xl:ml-auto xl:flex xl:w-auto xl:flex-1 xl:flex-wrap xl:justify-end xl:gap-x-8 xl:gap-y-4">
          ...
          <div className="min-w-0 xl:w-[280px]"><p className={labelCls}>{t("Location")}</p><button type="button" data-testid="location-field" onClick={handleOpenLocation} className="mt-1 flex h-10 w-full items-center justify-between rounded-xl border border-white/[0.09] bg-white/[0.04] px-3 text-left text-sm font-medium text-[#D8E3EC] transition-colors hover:border-cyan-300/20 hover:bg-white/[0.06] focus:outline-none focus:ring-2 focus:ring-[#22D3EE]/30"><span className="truncate">{loc ? formatLocation({ city: loc.name, province: loc.region, country: loc.country }) : t("Select location")}</span><MapPin className="ml-2 h-4 w-4 shrink-0 text-[#71869A]" strokeWidth={1.8} /></button></div>
          <div className="col-span-2 flex h-10 justify-end xl:col-span-1 xl:justify-start shrink-0 items-center rounded-xl border border-white/[0.09] bg-white/[0.04] p-1" aria-label="Language"><button type="button" onClick={() => setLanguage("id")} className={`h-8 min-w-[68px] rounded-lg px-3 text-center text-[10px] font-bold transition-colors ${language === "id" ? "bg-[#12324A] text-[#22D3EE]" : "text-[#71869A] hover:bg-white/[0.05] hover:text-white"}`}>Indonesia</button><button type="button" onClick={() => setLanguage("en")} className={`h-8 min-w-[68px] rounded-lg px-3 text-center text-[10px] font-bold transition-colors ${language === "en" ? "bg-[#12324A] text-[#22D3EE]" : "text-[#71869A] hover:bg-white/[0.05] hover:text-white"}`}>English</button></div>
        </div>
      </div>
