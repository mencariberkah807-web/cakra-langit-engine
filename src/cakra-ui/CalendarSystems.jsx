      ...
      <div className="grid grid-cols-1 gap-3.5 min-[480px]:grid-cols-2 md:grid-cols-3">
        {!data
          ? Array.from({ length: 7 }).map((_, i) => (
              <div
                key={i}
                className="h-[220px] animate-pulse rounded-xl border border-[#163A5C] bg-[#0A2139]"
              />
            ))
          : calendars.map((cal) => {
              ...
              return (
                <motion.div
                  ...
                  onKeyDown={
                    clickable
                      ? (e) => (e.key === "Enter" || e.key === " ") && openCalendar(cal.id)
                      : undefined
                  }
                  data-testid={`calendar-card-${cal.id}`}
                  className={`rounded-xl border p-4 transition-[border-color,background,box-shadow,transform] duration-200 ${
                    cal.future
                  ...
                  } ${clickable ? "cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#36C8FF] focus:ring-offset-2 focus:ring-offset-[#06172B]" : ""}`}
                >
                  <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#6F8CA6]">
                    Calendar
                  </p>
                  ...
                  {cal.future ? (
                    ...
                    <>
                      <p className="mt-3 text-[13px] font-semibold leading-tight text-[#DCEBFA]">
                        {cal.headline}
                      </p>
                      <p className="mt-0.5 text-[11px] uppercase tracking-wide text-[#7894AF]">
                        {cal.sub}
                      </p>
