...
export default function PublicNavigation({ isAuthenticated, settings }) {
  ...
  return (
    <header className="border-b border-white/10 bg-[#07111C] text-white">
      <nav className="mx-auto flex min-h-[76px] max-w-[1360px] items-center gap-8 px-5 sm:px-7 lg:px-10" aria-label="Navigasi utama">
        <a href="/" className="flex shrink-0 items-center gap-3" aria-label={`${siteName} Beranda`}>
          ...
          <span>
            <span className="block text-[12px] font-bold tracking-[0.06em] text-white">{siteName.toUpperCase()}</span>
            <span className="block text-[9px] text-slate-400">{tagline}</span>
          </span>
        </a>

        <div className="hidden items-center gap-7 lg:flex">
          {links.map(([label, href], index) => (
            <a key={label} href={href} className={`relative py-7 text-[11px] font-medium ${index === 0 ? 'text-cyan-300' : 'text-slate-300 hover:text-white'}`}>
              {label}
              {index === 0 && <span className="absolute inset-x-0 bottom-0 h-0.5 bg-cyan-400" />}
            </a>
          ))}
        </div>

        <div className="ml-auto flex items-center gap-2.5">
          <button type="button" aria-label="Cari" className="hidden h-9 w-9 items-center justify-center rounded-md text-slate-300 hover:bg-white/5 sm:flex">
            <Search className="h-4 w-4" strokeWidth={1.8} />
          </button>
          <a href={accountHref} className="rounded-md border border-cyan-300/25 bg-[#0D2535]/90 px-4 py-2 text-[11px] font-semibold text-white hover:bg-[#12324A]">
            {accountLabel}
          </a>
          {!isAuthenticated && (
